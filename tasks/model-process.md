...
## 단일 통합 흐름도 · 2026-09-10

사용자의 스크린샷 교정으로, 위의 두 도식 간 탐색 방식은 요구와 다름을 확인했다.
전체 흐름과 역할 내부 흐름을 하나의 SVG로 통합했다. 사용자 → PM → 역할 실행 →
QA 판정 → 수정/후속/성과 확인을 하나의 연결망으로 배치하고, 각 역할 내부 단계와
모델 후보를 같은 노드 안에 담았다. Paperclip 업무판과 Orca 실행 수단은 팀 옆에
연결했다. QA 초기 기준 협의, 기획→개발 직접 인계, 디자인↔개발 매핑 피드백,
QA→PM 귀환, 성과→다음 소재를 표시했다. 상세 역할 지침과 12개 인계 계약은 유지한다.

검증: Orca 1186×1550 viewport에서 최종 screenshot 시각 확인. SVG 1개, 역할 상세
5개, 인계 12행, 깨진 내부 anchor 0개, 페이지 가로 overflow 없음. 사용자→PM 선이
팀 배경에 가려진 부분을 수정하고 다시 렌더 확인했다. git diff --check 통과.
HTML과 이 작업 기록만 변경했으며 모델 배정·루프는 여전히 후보이고 runtime 변경은 없다.

## 연결선·여백 정리 · 2026-09-10

배정선을 같은 높이로 정렬하고 역할 박스 중심에 연결했다. 기획→개발 직접 인계의
교차는 비접속 브리지로 표시한다. 왼쪽의 긴 세로 라벨과 이중 귀환 통로를 제거하고
QA↔기획 초기 협의는 두 노드 사이에 배치했다. QA→PM은 별도 왼쪽 통로,
성과 학습·주기 점검→PM은 오른쪽 외곽 통로를 사용해 도구 호출선 교차를 없앴다.
SVG 최소 너비 제한을 해제해 좁은 창에서도 전체 도식이 보이도록 했다.

검증: Orca 911×1550 화면에서 정렬·라벨·전체 그림 렌더 확인, canvas overflow 없음.
업무 계약과 모델/runtime 상태는 변경하지 않았다. git diff --check 통과.

## README 이미지 · 2026-09-10

최종 HTML의 SVG를 Orca 브라우저에서 2880×2280 PNG로 렌더해
docs/images/model-process.png에 저장하고 README 상단에 삽입했다.
이미지 확대 링크와 원본 HTML 링크, 모델/루프의 제안 상태를 함께 표시한다.
PNG를 직접 열어 텍스트·연결선·잘림을 확인했다. 이미지에는 브라우저 UI나 로컬 경로가
포함되지 않는다. 향후 흐름도를 수정하면 PNG도 다시 렌더해야 한다.

## README 중심 프로젝트 안내 · 2026-09-10

사용자 요청으로 README를 프로젝트의 시작 문서로 정리했다. 소개와 통합 이미지에
역할/모델 후보 표, 7단계 업무 프로세스, 여러 제품의 맥락 구분과 도구 책임을 추가했다.
기존 설치·검증·외부 문서 의존성 안내는 유지하고 HTML은 보조 시각화로 연결한다.
역할 계약의 상세 정본은 skill reference에 유지한다. 문서 링크와 diff 검사를 수행했다.

## 마케팅·HR·성과 개선 루프 · 2026-09-10

사용자가 논의·조사에 기반한 전체 흐름 갱신을 요청했다. 단일 SVG에 마케팅의
고객·메시지·실험·측정, HR의 agent 준비·공동 리뷰·개선 인계를 추가했다.
역할 내부 흐름은 7개, 인계 표는 17행으로 확장했다. HR은 전문 판정을 단독 수행하지
않으며 PM·QA·직무와 리뷰하고, PM이 개선 배정·QA가 재검증한다. HR 자체 개선은
PM/사용자 검토다. roles reference, 운영 모델, README와 PNG를 함께 갱신했다.

docs/role-performance.md에 외부 조사 근거와 우리 지표 후보를 구분했다. 제품 성과·
산출물·효율, 표본/분모·관찰 기간·환경/버전·unknown을 분리한다. 지표 목표치·리뷰
주기·자동 실행은 미확정/미활성이며 managed runtime skill 재등록이나 agent 실행은 하지 않았다.

검증: Orca PC 렌더와 2880×2800 PNG 직접 확인. SVG 1개·역할 7개·인계 17행,
canvas overflow 없음. skill validator 및 기존 계약 5개 검사 통과, fixture 변경 없음.
문서 상대 링크와 git diff --check 확인. 설치/다른 제품/공통 wiki는 변경하지 않았다.

## 전체 구도 재배치 · 2026-09-10

사용자가 실제 이미지를 보고 재배치할 것을 요청했다. 마케팅을 옆에 덧붙인 비정형
경계를 없애고 상단 입력/도구 → PM → 동일 크기의 기획·디자인·개발·마케팅 → QA →
HR/제품 성과로 전체 구도를 다시 구성했다. 실행 역할은 같은 크기·높이·간격으로 정렬,
QA·HR의 귀환은 왼쪽 외곽, 성과 학습은 오른쪽 외곽 통로로 분리했다.
상세 인계 17행은 유지하며 기획→개발 직접 인계 등은 노드 설명과 상세 표에 보존했다.

수정 전 PNG, Orca의 변경 후 PC 화면, 최종 3200×2760 PNG를 직접 확인했다.
연결 라벨이 수직선을 가로지르던 부분과 PM/QA 폭을 조정했다. README 이미지도 갱신.
이번 변경은 표현 배치만이며 역할 계약·자동 실행 상태는 바꾸지 않았다. diff 검사 통과.
