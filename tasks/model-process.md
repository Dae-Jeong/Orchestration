...
## 전체 흐름과 역할 상세 연결 · 2026-09-10

사용자 교정: 역할 상세가 기존 전체 흐름을 대체하면 안 된다. 기존 PM/Paperclip/Orca
흐름도를 복원하고 그 역할 노드에서 같은 페이지의 역할 내부 흐름으로 연결했다.
전체 흐름 → 실행 팀 상세 → 인계 계약 → 주기 점검의 탐색과 전체로 돌아가기 링크를 추가했다.
두 SVG marker ID는 분리했다. 기존 5개 역할·25단계·12개 인계는 유지했다.

검증: Orca에서 전체 흐름의 디자인 노드 클릭 → #role-design 이동/강조,
전체 복귀 링크의 DOM activation → #overview 이동 확인. 전체와 상세가 연결된 PC 렌더
검토 및 내부 anchor 대상·중복 ID·whitespace 검사 통과.
